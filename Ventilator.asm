...
; Define variables
    ...
    TOTALINTERRUPTSLOW ;0xA	total interrupts of timer (8 breaths)
    TOTALINTERRUPTSHIGH ;0xB	total interrupts of timer (8 breaths)
    TIMERINTERRUPTS ;0xC	total interrupts of timer (1 breath)
    INTERRUPTSPERBREATH ;0xD	Number of itnerrupts per 1 breath
    REACHEDHIGH; 0xE
...
; ------ Set up all variables -------
	...
	CLRF LOWTHRESHOLD
	CLRF HIGHTHRESHOLD
	CLRF TIMERINTERRUPTS
	CLRF TOTALINTERRUPTSHIGH
	CLRF TOTALINTERRUPTSLOW
	CLRF ADCRESULT
	MOVLW   0x08 ; 8 values to read
...
FiveLoop1   CALL FiveDelay
	    ...
	    MOVLW   0x23; -25 from the adc value
...
FiveLoop    CALL FiveDelay
	    DECFSZ  FiveCounter
	    GOTO FiveLoop
	    ; have now waited so now take low threshold
	    BSF ADCON0,GO ; Start ADC conversion
	    BTFSC ADCON0,GO ; If conversion done skip the next line
...
BREATHDETECT
    ...
    BSF T2CON,TMR2ON ; Start LOW/HIGH PWM
    ;;------- Setup PORTA for ADC LED output ----------
...
;---------- Setup PWM on Port RC2----------
	    ...
	    ; Delay to prevent accidental ADC convert from calibration
	    MOVLW   0x1E ; 100 repetitions
	    MOVWF   FiveCounter
FiveLoop2   CALL FiveDelay
	    DECFSZ  FiveCounter
	    GOTO FiveLoop2
...
TableWrite
	    ; now write TIMERINTERRUPTS to table
	    MOVF    TIMERINTERRUPTS,0 ; W = Timerrolls
	    MOVWF   TABLAT
...
TableLoop   
	    ...
	    ADDWF   TOTALINTERRUPTSLOW ; adding value to TOTALINTERRUPTSLOW
	    BNC	    DecrementTC ; there is no carry
	    INCF    TOTALINTERRUPTSHIGH; there is a carry
DecrementTC 
	    DECF    TABLECOUNT,1
	    MOVLW   0x0
...
Averaging
	    ; TOTALINTERRUPTSLOW and TOTALINTERRUPTSHIGH have all the rolls in them now
	    ...
	    RRNCF TOTALINTERRUPTSLOW
	    RRNCF TOTALINTERRUPTSLOW
	    RRNCF TOTALINTERRUPTSLOW
	    MOVLW B'00011111'
	    ANDWF TOTALINTERRUPTSLOW
	    RRNCF TOTALINTERRUPTSHIGH
	    RRNCF TOTALINTERRUPTSHIGH
	    RRNCF TOTALINTERRUPTSHIGH
	    BTFSC TOTALINTERRUPTSHIGH,7
	    BSF TOTALINTERRUPTSLOW,7
	    BTFSC TOTALINTERRUPTSHIGH,6
	    BSF TOTALINTERRUPTSLOW,6
	    BTFSC TOTALINTERRUPTSHIGH,5
	    BSF TOTALINTERRUPTSLOW,5 
	    MOVLW B'00011111'
	    ANDWF TOTALINTERRUPTSHIGH
	    ; successfuly divided whole of totalRolls by 8
	    ; totalRolls now represents rollovers taken for one breath
	    ; only have to work with TOTALINTERRUPTSLOW in PICLEDS cause if value above a certain threshold the person is dead anyway
	    MOVFF TOTALINTERRUPTSLOW,INTERRUPTSPERBREATH; timer rollovers per breath (average)
	    CALL PICLEDS
...
PICLEDS
	; Check value of INTERRUPTSPERBREATH and set various PIC LEDs accordingly		
Greater76
	    
	    MOVLW 0x4C ; 76 rolls
	    CPFSGT INTERRUPTSPERBREATH ; skip next line if INTERRUPTSPERBREATH > 76
	    GOTO Greater57 ; INTERRUPTSPERBREATH < 76
	    MOVLW   b'00000000' ; INTERRUPTSPERBREATH > 76
	    MOVWF PORTA ; All LEDs on PORTA off
	    GOTO RETURNLEDS
Greater57
	    MOVLW 0x39 ; 57 rolls
	    CPFSGT INTERRUPTSPERBREATH ; skip next line if INTERRUPTSPERBREATH > 57
	    GOTO Greater46 ; INTERRUPTSPERBREATH < 57
	    MOVLW   b'10000000' ; INTERRUPTSPERBREATH > 57
	    MOVWF PORTA ; 1 LEDs on PORTA on
	    GOTO RETURNLEDS
	    
Greater46
	    MOVLW 0x2E ; 46 rolls
	    CPFSGT INTERRUPTSPERBREATH ; skip next line if INTERRUPTSPERBREATH > 46
	    GOTO Greater30 ; INTERRUPTSPERBREATH < 46
	    MOVLW   b'11000000' ; INTERRUPTSPERBREATH > 46
	    MOVWF PORTA ; 2 LEDs on PORTA on
	    GOTO RETURNLEDS
Greater30
	    MOVLW 0x1E ; 30 rolls
	    CPFSGT INTERRUPTSPERBREATH ; skip next line if INTERRUPTSPERBREATH > 30
	    GOTO Less30 ; ROLLSPERBINTERRUPTSPERBREATHREATH < 30
	    MOVLW   b'11100000' ; INTERRUPTSPERBREATH > 30
	    MOVWF PORTA ; 3 LEDs on PORTA on
	    GOTO RETURNLEDS
Less30
	    MOVLW   b'11110000' ; INTERRUPTSPERBREATH < 30
...
PWMHIGH
	    MOVLW .61
	    MOVWF PR2
	    MOVLW .12 ; duty cycle at 20% : 0.2 * PR2 = 0.2 * 61 = 12.2
	    MOVWF CCPR1L
	    BCF	CCP1CON,5
	    BSF	CCP1CON,4 ; 0.25
	    RETURN
...
Timer4Interrupt
	...
	INCF TIMERINTERRUPTS ; Add one rollover count
	BCF	PIR5,TMR4IF
InterruptReturn
	RETFIE
    end